        ...
        stage('Create containers and run test') {
            steps {
                script {
                    ...
                    sh "mvn test -Dcucumber.filter.tags=\"@Google or @Shopee\" -Dcucumber.filter -Dbrowser=${params.BROWSER} -Dhostname=${ipAddress} -DexecutingEnv=test -DtestedEnv=uat -Dplatform=desktop"
                    sh 'ls -al'
